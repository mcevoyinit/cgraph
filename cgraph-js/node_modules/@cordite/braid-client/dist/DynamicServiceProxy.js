...
/**
                ...
                *
                *  example: {
                *    url: 'https://localhost:8080/api/jsonrpc',
                *    credentials: {
                *      username: 'foo',
                *      password: 'bar'
                *    }
...
function DynamicProxy(config, serviceName, onOpen, onClose, onError, options) {_classCallCheck(this, DynamicProxy);
  ...
  function getDocs(fn) {
    ...
    for (var idx in fn.__metadata) {
      ...
      var apifn = "* " + methodDefinition.name + '(' + generateParamList(methodDefinition) + ') => ' + JSON.stringify(methodDefinition.returnType) + '\n';
  ...
  function generateParamDocs(methodDefinition) {
    ...
    map(function (p) {
      return '  @param ' + p + ' - ' + JSON.stringify(methodDefinition.parameters[p]);
    }).join('\n');
  }
  // --- PUBLIC FUNCTIONS ---
